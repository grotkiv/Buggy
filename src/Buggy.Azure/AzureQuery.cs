...
public class AzureQuery : IWorkItemQuery
{
    ...
    public AzureQuery(IOptions<AzureProject> options, ILogger<AzureQuery> logger)
    {
        ...
        this.organizationUrl = new Uri(string.Join('/', project.Url, project.Organization));
        this.logger = logger;

        logger.LogInformation("Azure URI: {@AzureUri}", organizationUrl);
    ...
    public async Task<IEnumerable<Model.WorkItem>> RunQuery()
    {
        ...
        using var httpClient = new WorkItemTrackingHttpClient(this.organizationUrl, credentials);
